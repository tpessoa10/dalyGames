        ...
        <div className="w-full bg-gray-900 p-4 h-44 text-white rounded-lg flex justify-between flex-col">
            {showInput ? (
                <div className='flex items-center justify-center gap-3'>
                    <input className='w-full rounded-md h-8 text-black px-2 bg-white' onChange={(e) => setInput(e.target.value)} value={input} type='text'/>
                    <button>
                        <FiX onClick={handleButton} size={24} color='#fff'/>
                    </button>
                </div>
            ) : (
